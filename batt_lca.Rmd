...
First, we load up data on power plants from the electrical grid of interest, based on territory name. We will focus on the nameplate capacities and CO2-equivalent emissions rates of these plants, though the full dataset and accompanying documentation for the US EPA's eGrid2012[@eGRID2012] data are available [here](https://www.epa.gov/energy/emissions-generation-resource-integrated-database-egrid).

The code below also assigns marginal costs to each power plant by fuel type and generator type, calculated based on methods[@Gilmore2010] in the literature and publicly available data[@EIA2014b, @EIA2015, @EIA2014a]. In so doing, we estimate the priority with which power plants fire. Documented capacity factors may also be incorporated into future iterations of the grid dispatch model.
...
Depending on the level of cumulative demand for electricity, grid operators allow bids for energy to be supplied until enough are submitted to satisfy load. At this point, the grid operator will pay all accepted bidders the highest marginal cost among all accepted bids.
...
tabsetPanel(
  tabPanel("Graph",
          ...
          renderPlot({units <- ifelse(input$var2 == "mc_rand",
                                      "$ / kWh",
                                      "g CO2eq / kWh")
  ...
  tabPanel("Weekly",
         ...
         output$year_plot <- renderPlot({
           nyiso_load.week <- nyiso_load.min %>%
             select(contains("mw"), wk, date_time) %>%
