...
=======
library(dygraphs)
>>>>>>> 59baa622791f56cfd3e13c6fe13f823c0671c4f2
library(plyr)
library(tidyverse)
library(lubridate)
library(reshape2)
    ...
    output$load_plot <- renderDygraph({
      nyiso_load.min <- nyiso_load.min %>%
        ...
        filter(run == "plc2erta_3",
               wk >= 28 & wk <= 32) %>% 
        mutate(roll_sd = RcppRoll::roll_sdr(mw, 540),
               upr = mw + roll_sd,
               lwr = mw - roll_sd) %>%
        na.omit()
...
nyiso_load.xts <- reactive({
  nyiso_ts <- nyiso_load.min %>%
    filter(run == "plc2erta_1") %>% 
    select(wk, date_time, mw) %>%
    mutate(mw_daily_avg = RcppRoll::roll_meanr(x = mw, n = 288, fill = NA)) %>%
    fill(mw_daily_avg, .direction = "up") %>%
    select(-wk)
  
  nyiso_ts <- right_join(nyiso_ts,
                         data.frame(date_time = seq.POSIXt(nyiso_ts$date_time[1],
                           nyiso_ts$date_time[nrow(nyiso_ts)],
                           by="5 mins")), by = "date_time")
  
  # rownames(nyiso_ts) <- nyiso_ts[["date_time"]]
  xts(select(nyiso_ts, -date_time), nyiso_ts$date_time, by="5 mins")
})

tabsetPanel(
  tabPanel("Weekly Load",
    output$load_dygraph <- renderDygraph({
      dygraph(nyiso_load.xts(),
              ylab = "MW") %>% 
        dySeries("mw", color = cbb_qual.n[["Petro-fuels"]]) %>%
        dySeries("mw_daily_avg", color = cbb_qual.n[["PV"]], strokeWidth = 2) %>% 
        dyRangeSelector() %>% 
